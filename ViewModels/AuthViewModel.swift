...
class AuthViewModel: ObservableObject {
    ...
    func register(email: String, password: String, fullName: String) {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { [weak self] result, error in
            ...
            let newUser = UserModel(id: user.uid, name: fullName, email: email, role: .debater, isActive: true, createdAt: Date())
            ...
            DispatchQueue.main.async {
                ...
                if let data = snapshot?.data(),
                   ...
                   let email = data["email"] as? String {
                    ...
                    // Asumsikan enum UserRole punya inisialisasi rawValue String
                    let role = UserRole(rawValue: roleStr) ?? .debater
                    let isActive = data["isActive"] as? Bool ?? true
                    let createdAt = (data["createdAt"] as? Timestamp)?.dateValue() ?? Date()
                    
                    self?.currentUser = UserModel(id: id, name: name, email: email, role: role, isActive: isActive, createdAt: createdAt)
                }
            }
        }
    }
