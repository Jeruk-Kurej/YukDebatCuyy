                    ...
                    VStack(spacing: 24) {
                        ...
                        VStack(spacing: 16) {
                            ...
                            VStack(spacing: 4) {
                                Text(authVM.currentUser?.name ?? "Memuat...")
                                    .font(.title2.bold()).foregroundStyle(Color.textCharcoal)
                            ...
                            .font(.caption.bold()).foregroundStyle(Color.btnPositive)
                                ...
                                Button(action: { showAdjudicatorForm = true }) {
                                    HStack(spacing: 16) {
                                        Image(systemName: "briefcase.fill")
                                            .font(.title3).foregroundStyle(Color.accentWalnut).frame(width: 24)
                                        Text(adjReqVM.hasPendingRequest ? "Pengajuan Juri (Pending)" : "Daftar Menjadi Juri")
                                            .font(.system(.body, design: .default, weight: .medium))
                                            .foregroundStyle(Color.textCharcoal)
                                        Spacer()
                                        if adjReqVM.hasPendingRequest {
                                            Text("Menunggu").font(.caption.bold()).foregroundStyle(.orange)
                                        } else {
                                            Image(systemName: "chevron.right")
                                                .font(.caption.bold()).foregroundStyle(Color.gray.opacity(0.5))
                                        }
                                    }
                                    .padding()
                                }
                        ...
                        Button(action: { showLogoutAlert = true }) {
                            Text("Keluar (Log Out)").font(.headline)
                                ...
                                .background(Color.btnNegative).clipShape(RoundedRectangle(cornerRadius: 16))
                        }
                        .alert("Keluar", isPresented: $showLogoutAlert) {
                            Button("Batal", role: .cancel) {}
                            Button("Keluar", role: .destructive) { authVM.logout() }
                        }
                    }
                    // PERBAIKAN HIG: Kurangi padding atas secara signifikan
                    .padding(.horizontal, 24)
                    .padding(.top, 16)
                    .padding(.bottom, 120) // Ruang lega untuk Custom Tab Bar
                }
            }
            .navigationTitle("Profil Saya")
            .onAppear { adjReqVM.checkExistingRequest() }
            .sheet(isPresented: $showAdjudicatorForm) { ApplyAdjudicatorFormView(viewModel: adjReqVM) }
            .modernToast(message: $adjReqVM.statusMsg, isError: adjReqVM.statusMsg?.contains("Gagal") == true)
        }
    }
}
